...
:: Define hashcat path (since the bat file is in the hashcat directory)
...
set HASH_FILE=sak.hc22000
set OUTPUT_FILE=cracked.txt

:: Read masks from mask.txt
for /F "tokens=*" %%A in (masks.txt) do (
    echo Running mask: %%A
    %HASHCAT_PATH% %CHARSET% -a 3 -m 22000 %HASH_FILE% %%A -o %OUTPUT_FILE% --potfile-disable -d 1
